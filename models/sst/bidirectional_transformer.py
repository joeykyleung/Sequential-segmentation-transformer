...
class Encoder(nn.Module):
    ...
    """
    def __init__(self, dim=768, hidden_dim=3072, num_heads=8):
        super(Encoder, self).__init__()
        # self.MultiHeadAttention = MultiHeadAttention(dim)
        self.MultiHeadAttention = nn.MultiheadAttention(dim, num_heads=num_heads, batch_first=True, dropout=0.1)
...
class BidirectionalTransformer(nn.Module):
    def __init__(self, args):
        ...
        self.tok_emb = nn.Embedding(self.num_codebook_vectors + 2, args.dim)
        ...
        # self.pos_emb = nn.init.trunc_normal_(nn.Parameter(torch.zeros(2 * self.num_image_tokens + 1, args.dim)), 0., 0.02)
        # self.register_buffer("pos_emb", nn.init.trunc_normal_(nn.Parameter(torch.zeros(1024, args.dim)), 0., 0.02))
        self.blocks = nn.Sequential(*[Encoder(args.dim, args.hidden_dim, args.n_heads) for _ in range(args.n_layers)])
        self.Token_Prediction = nn.Sequential(*[
            nn.Linear(in_features=args.dim, out_features=args.dim),
            nn.GELU(),
            nn.LayerNorm(args.dim, eps=1e-12)
        ])
        self.bias = nn.Parameter(torch.zeros(self.num_image_tokens+1, self.num_codebook_vectors + 2))
