...
class Decoder(nn.Module):
    def __init__(self, args, is_mask=False):
        ...
        if not is_mask:
            ...
            layers.append(nn.Conv2d(block_in, 4, kernel_size=3, stride=1, padding=1))

        self.model = nn.Sequential(*layers) # nn.Sigmoid() for image, nn.Softmax(dim=1) for label
