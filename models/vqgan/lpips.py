...
class LPIPS(nn.Module):
    ...
    def load_from_pretrained(self, name="vgg_lpips"):
        ckpt = get_ckpt_path(name, "/homes/jkl223/Desktop/Individual Project/models/checkpoints")
...
class ScalingLayer(nn.Module):
    def __init__(self):
        ...
        avg_shift = torch.Tensor([-.030, -.088, -.188]).mean()      # original values for RGB
        avg_scale = torch.Tensor([.458, .448, .450]).mean()
        self.register_buffer("shift", torch.Tensor([avg_shift])[None, :, None, None])
        self.register_buffer("scale", torch.Tensor([avg_scale])[None, :, None, None])
...
class VGG16(nn.Module):
    def __init__(self):
        ...
        vgg_pretrained = vgg16(weights=VGG16_Weights.DEFAULT)
        
        # convert first layer to accept 1 channel
        pretrained_weights = vgg_pretrained.features[0].weight.data
        ...
        first_conv_layer.weight.data = averaged_weights
        
        # first_conv_layer.weight.data[:, 1:, :, :].zero_()  # or .normal_(0, 0.01)
        vgg_pretrained.features[0] = first_conv_layer
...
if __name__ == '__main__':
    real = torch.randn(10, 1, 256, 256) #(10, 3, 256, 256)
    fake = torch.randn(10, 1, 256, 256)
    loss = LPIPS().eval()
