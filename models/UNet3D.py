...
class Config:
    ''' Configurations for the 3D UNet '''
    name = 'UNET3D'
    slice_size = (128, 128, 21)

    batch_size = 12
    loss_type = 'diceCE' #'dice' #'crossentropy'
    optimizer = 'adam'
    learning_rate = 1e-4
...
if __name__ == '__main__':
    ...
    x = torch.rand(1, 1, *size)  # batch of 1 MRI volume: 1 channel, 64 x 64 x 64 voxels
    model = UNet3D()
    preds = model(x)
    ...
    summary(model, (1, *size)) #21 is max z depth of dataset
