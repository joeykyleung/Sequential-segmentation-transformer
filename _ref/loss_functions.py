# Adapted from "An Exploration of 2D and 3D Deep Learning Techniques for Cardiac MR Image Segmentation"
...
#     def forward(self, preds, targets):
#         # Assuming preds are logits and need to be converted to probabilities
#         if preds.dim() != targets.dim():
#             raise ValueError("Predictions and targets must have the same number of dimensions")
        
#         preds = torch.sigmoid(preds)  # Convert logits to probabilities if not already done
#         preds = preds.contiguous()
#         targets = targets.contiguous()

#         intersection = (preds * targets).sum(dim=(1, 2, 3, 4))  # Sum over spatial dimensions
#         union = preds.sum(dim=(1, 2, 3, 4)) + targets.sum(dim=(1, 2, 3, 4))  # Sum over spatial dimensions
#         dice = (2. * intersection + self.smooth) / (union + self.smooth)  # Compute Dice coefficient
